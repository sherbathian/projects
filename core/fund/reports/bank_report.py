...
import datetime
from fund.models import Bank
from django.http import HttpResponse
from django.contrib import messages
import io


# Custom admin view for Bank reports
class BankReportAdminView:
    @staticmethod
    def dashboard_view(request):
        ...
        base_qs = Bank.objects.all()
        
        if year_param != 'any':
            try:
                base_qs = base_qs.filter(date__year=int(year_param))
            except ValueError:
                pass
        
        try:
            field_type = Bank._meta.get_field('date').get_internal_type()
        ...
        try:
            if start_date:
                ...
                if is_datetime:
                    base_qs = base_qs.filter(date__date__gte=sd)
                else:
                    base_qs = base_qs.filter(date__gte=sd)
        except Exception:
            pass
        ...
        try:
            if end_date:
                ...
                if is_datetime:
                    base_qs = base_qs.filter(date__date__lte=ed)
                else:
                    base_qs = base_qs.filter(date__lte=ed)
        except Exception:
            pass
        ...
        # dropdowns for years
        years_qs = Bank.objects.dates('date', 'year').distinct()
        ...
        return TemplateResponse(request, "admin/bank/report.html", context)

    @staticmethod
    def export_pdf(request):
        """Export aggregated Bank rows (filtered) as PDF."""
        start_date = request.GET.get('start_date', '')
        end_date = request.GET.get('end_date', '')
        year_param = request.GET.get('year', 'any')
        ...
        elements = []
        title = Paragraph("Bank Export", styles['Title'])
        ...
        resp['Content-Disposition'] = 'attachment; filename="bank_export.pdf"'
        return resp
